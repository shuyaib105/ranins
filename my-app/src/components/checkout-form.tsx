...
export function CheckoutForm({ cart, totalAmount, onSuccess }: CheckoutFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="rounded-[24px] bg-gray-50 p-6 text-center">
      ...
      <FieldGroup className="gap-6">
        ...
        <Field>
          ...
          >
            <ToggleGroupItem
              value="bkash"
              className="flex-1 rounded-2xl border-2 py-8 font-black data-[state=on]:border-pink-500 data-[state=on]:bg-pink-50 data-[state=on]:text-pink-600"
            >
              bKash
            </ToggleGroupItem>
            <ToggleGroupItem
              value="nagad"
              className="flex-1 rounded-2xl border-2 py-8 font-black data-[state=on]:border-orange-500 data-[state=on]:bg-orange-50 data-[state=on]:text-orange-600"
            >
              Nagad
            </ToggleGroupItem>
          </ToggleGroup>
          <FieldDescription className="mt-2 text-center text-[11px] font-medium leading-relaxed">
            Send ৳ {totalAmount} to <span className="font-bold text-black">{paymentNumber}</span> (Personal) 
        ...
        <Field>
          ...
          <Input
            ...
            required
            className="rounded-2xl border-2 border-black/10 focus-visible:border-black"
          />
        </Field>
      </FieldGroup>
